...
describe('Vesting', function () {
    ...
    const nonVestedAmount= distributedAmount.div(10);
    const vestedAmount = distributedAmount.sub(nonVestedAmount);
    let snapshotId;
    ...
    beforeEach(async function () {
        ...
        const Vesting = await ethers.getContractFactory('VestingWithInitialUnlock');
    ...
    describe('General Contract checks', function () {
        ...
        it('should have totalDistributedBalance', async function () {
            expect(await this.vesting.vestedBalance()).to.be.equal('450000000000000000000000');
            expect(await this.vesting.nonVestedBalance()).to.be.equal(distributedAmount.div(10));
        })
    ...
    describe('Contract Tests', function () {
        ...
        it('should send non-vested amount on first epoch', async function () {
            await this.wtf.mint(this.vesting.address, distributedAmount); // set tokens
            await moveAtEpoch(1)
            await this.vesting.connect(this.beneficiary1).claim();
            expect(await this.wtf.balanceOf(this.beneficiary1.address)).to.be.equal(nonVestedAmount);
            expect(await this.vesting.balance()).to.be.equal(vestedAmount);
            expect(await this.vesting.lastClaimedEpoch()).to.be.equal(0);
        })
        it('should mint for 1 week', async function () {
            ...
            await moveAtEpoch(2)
            await this.vesting.connect(this.beneficiary1).claim();
            expect(await this.wtf.balanceOf(this.beneficiary1.address)).to.be.equal(vestedAmount.div(104).add(nonVestedAmount));
            expect(await this.vesting.balance()).to.be.equal(distributedAmount.sub(vestedAmount.div(104)).sub(nonVestedAmount));
